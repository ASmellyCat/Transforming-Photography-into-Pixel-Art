...
class ResnetGenerator_our(nn.Module):
    ...
    def forward(self, input):
        ...
        def process_image(image, start_idx, end_idx):
            return image[:, start_idx:end_idx, :, :]

        def process_attention(attention, start_idx, end_idx):
            attention_slice = attention[:, start_idx:end_idx, :, :]
            attention_repeat = attention_slice.repeat(1, 3, 1, 1)
            return attention_repeat

        def main_function(self, content, x):
            image = self.tanh(content)
            image_slices = [process_image(image, i * 3, (i + 1) * 3) for i in range(10)]

            x_attention = F.relu(self.deconv2_norm_attention(
                self.deconv2_attention(F.relu(self.deconv1_norm_attention(self.deconv1_attention(x))))))

            attention = self.deconv3_attention(x_attention)
            attention_softmax = F.softmax(attention, dim=1)

            attention_slices = [process_attention(attention_softmax, i, i + 1) for i in range(10)]

            output_slices = [image_slices[i] * attention_slices[i] for i in range(10)]

            o = sum(output_slices)

            return o, *output_slices, *attention_slices, *image_slices
